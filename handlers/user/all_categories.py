...
async def create_category_buttons(page: int):
    categories = CategoryService.get_unsold(page)
    if categories:
...
async def create_subcategory_buttons(category_id: int, page: int):
    current_level = 1
    items = ItemService.get_unsold_subcategories_by_category(category_id, page)
    subcategories_builder = InlineKeyboardBuilder()
    for item in items:
        subcategory_price = ItemService.get_price_by_subcategory(item.subcategory_id)
        available_quantity = ItemService.get_available_quantity(item.subcategory_id)
...
async def select_quantity(callback: CallbackQuery):
    ...
    description = ItemService.get_description(subcategory_id)
...
async def buy_processing(callback: CallbackQuery):
    ...
    is_in_stock = ItemService.get_available_quantity(subcategory_id) >= quantity
