...
import config
from db import session_execute, session_commit, session_refresh
...
class BuyService:
    ...
    @staticmethod
    async def get_buys_by_buyer_id(buyer_id: int, page: int, session: Union[AsyncSession, Session]):
    ...
    @staticmethod
    async def get_max_page_purchase_history(buyer_id: int, session: Union[AsyncSession, Session]):
    ...
    @staticmethod
    async def insert_new(user: User, quantity: int, total_price: float, session: Union[AsyncSession, Session]) -> int:
    ...
    @staticmethod
    async def get_not_refunded_buy_ids(page: int, session: Union[AsyncSession, Session]):
    ...
    @staticmethod
    async def refund(buy_id: int, refund_data: RefundBuyDTO, session: Union[AsyncSession, Session]):
        await UserService.reduce_consume_records(refund_data.user_id, refund_data.total_price, session)
        stmt = update(Buy).where(Buy.id == buy_id).values(is_refunded=True)
        await session_execute(stmt, session)
        await session_commit(session)

    @staticmethod
    async def get_max_refund_pages(session: Union[AsyncSession, Session]):
    ...
    @staticmethod
    async def get_new_buys_by_timedelta(timedelta_int, session: Union[AsyncSession, Session]):
