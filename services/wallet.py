...
class WalletService:
    ...
    @staticmethod
    async def request_crypto_address(callback_data: WalletCallback, state: FSMContext) -> tuple[str, InlineKeyboardBuilder]:
        kb_builder = InlineKeyboardBuilder()
        kb_builder.row(AdminConstants.back_to_main_button)
    ...
    @staticmethod
    async def withdraw_transaction(callback_data: WalletCallback, state: FSMContext) -> tuple[str, InlineKeyboardBuilder]:
        state_data = await state.get_data()
        kb_builder = InlineKeyboardBuilder()
        withdraw_dto = await CryptoApiWrapper.withdrawal(
        ...
        withdraw_dto = WithdrawalDTO.model_validate(withdraw_dto, from_attributes=True)
        [kb_builder.button(
            text=Localizator.get_text(BotEntity.ADMIN, "transaction"),
